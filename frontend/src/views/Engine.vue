  ...
  <div class="page engine-page">
    ...
    <div class="card">
      <el-alert type="info" :closable="false" show-icon>
        ...
        <div class="boundary">
          <p>当前版本已支持并通过真实作答数据验证：客观题全题型与公式题（符号 / 数值抽样判等价）确定性判分、文本主观题「量规 → 证据 → 判点 → 大模型评语」协同、口语题四层表现报告（faster-whisper 接入后流畅层按词级时间戳真算；发音层未接入音素对齐时如实标「无数据」）、<b>图片作答 OCR</b>（视觉模型优先，失败回退本地 RapidOCR，转文字后按主观题判分）、<b>实操视频基础版</b>（服务端转写音轨，按步骤量规关键词判内容覆盖，动作/时序步骤证据不足转人工）、质量门控「空答不静默给分、转人工复核」、教师复核 / <b>双评（两位教师背靠背独立评分，超差进入仲裁）</b> / 发布双人复核 / 全程审计、Q 矩阵与 60 维能力诊断 + BKT 时序掌握 + 画像-作答匹配度，以及含延迟、客观题守门准确率、engine↔教师一致率、分数段校准的质量看板。评阅默认由<b>内置本地引擎</b>完成，开箱即用；在服务配置（<code>.env</code>）中填入 DeepSeek / 通义 / 智谱的 API Key 后自动启用外部大模型做判点复核与评语生成。</p>
