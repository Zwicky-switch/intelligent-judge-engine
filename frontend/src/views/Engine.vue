  ...
  <div class="page engine-page">
    ...
    <div class="card">
      <el-alert type="info" :closable="false" show-icon>
        ...
        <div class="boundary">
          <p>当前版本已支持并通过真实作答数据验证：客观题全题型与公式题（符号 / 数值抽样判等价）确定性判分、文本主观题「量规 → 证据 → 判点 → 大模型评语」协同、口语题四层表现报告（发音 / 流畅层在未接入 ASR 评测源时如实标记「无数据」，支持文本转写作答与录音证据留存）、质量门控「空答不静默给分、转人工复核」、教师复核 / <b>双评（两位教师背靠背独立评分，超差进入仲裁）</b> / 发布双人复核 / 全程审计、Q 矩阵与 60 维能力诊断 + BKT 时序掌握 + 画像-作答匹配度，以及含延迟、客观题守门准确率、engine↔教师一致率、分数段校准的质量看板。评阅默认由<b>内置本地引擎</b>完成，开箱即用；在服务配置（<code>.env</code>）中填入 DeepSeek / 通义 / 智谱的 API Key 后自动启用外部大模型做判点复核与评语生成。</p>
          <p>以下能力属于<span class="phase2">后续版本规划</span>，当前版本已完成接口预留、不产生无效判定：实操视频姿态识别、实时音素级发音评测（ASR）、大规模真实题库的 IRT / BKT 参数标定与训练。</p>
        </div>
      </el-alert>
    </div>
  </div>
...
const meta = ref(null)
const isLive = computed(() => !!meta.value && isExternalProvider(meta.value.llm_provider) && meta.value.has_api_key)
